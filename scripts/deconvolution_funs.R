...
createSigMatrix <- function(mutations.vector, mutation_sheet) {
  ...
  msig <- setNames(
    data.frame(matrix(ncol = ncol(mutations.df) + 1, nrow = 0)),
    c("WT", colnames(mutations.df))
  )
...
simulateWT <- function(mutations.vector, bulk_freq.vector,
                       simple_sigmat.dataframe, coverage.vector) {
  ...
  # 1. make "WT mutations"
  muts_wt <- lapply(mutations.vector, function(x) {
    str_replace(
      x, regex(".$"),
  ...
  # from 1
  bulk_wt <- lapply(bulk_freq.vector, function(x) {
    1 - x
  })
...
dedupeVariants <- function(variant, variants.df, dedup_variants.df) {
  ...
  for (row in dedup_variants.df$variants) {
    if (grepl(row, groupName_variants)) {
      # if variants are getting pooled with WT they are just WT and nothing else
      # and also not "others"
