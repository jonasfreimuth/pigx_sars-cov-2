...
```{r load_data}
mutations_sig_unfiltered <- read.csv(params$unfiltered_mutation_sig_file)
...
if (nrow(mutations_sig) > 0) {
  ...
  mutations_sig <- mutations_sig %>%
    ...
    mutate(sigflag = ifelse(any(
      sigmuts_df == str_split(mutation, "::")[[1]][2]
    ), "SigMut", ""))
...
melted <- melted %>%
  ...
  rowwise() %>%
  mutate(sigflag = ifelse(any(
    sigmuts_df == str_split(variable, "::")[[1]][2]
  ), "SigMut", "")) %>%
  ungroup()
...
melted <- reshape2::melt(filtered_approved_mut_plot,
  ...
  rowwise() %>%
  mutate(sigflag = ifelse(any(sigmuts_df == str_split(variable, "::")[[1]][2]),
    "SigMut", NULL
  ))
