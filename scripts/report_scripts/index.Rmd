...
sigmuts_df <- fread(mutation_sheet) %>%
  ...
  mutate_all(funs(str_replace(., str_glue('^[^{aa_sep}]*{aa_sep}', ''))))
...
if (nrow(mutations_sig) > 0) {
  ...
  mutations_sig <- mutations_sig %>%
    ...
    mutate(sigflag = ifelse(any(
      sigmuts_df == str_split(mutation, str_glue("{aa_sep}{aa_sep}")[[1]][2]
    ), "SigMut", "")))
...
melted <- melted %>%
  ...
  rowwise() %>%
  mutate(sigflag = ifelse(any(
    sigmuts_df == str_split(variable, str_glue("{aa_sep}{aa_sep}"))[[1]][2]
  ), "SigMut", "")) %>%
  ungroup()
...
melted <- reshape2::melt(filtered_approved_mut_plot,
  ...
  mutate(
    sigflag = ifelse(
      any(sigmuts_df == str_split(
        variable, str_glue("{aa_sep}{aa_sep}")
      )[[1]][2]),
      "SigMut", NULL
    )
  )
