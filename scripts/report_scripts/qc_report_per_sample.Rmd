...
mut_cov_df <- data.frame(
  "Total number of tracked mutations" = coverage_df[1],
  "Number of mutations covered" = coverage_df[2],
  "Mutation locations (NT) not covered" = coverage_df[3],
  check.names = FALSE
)

datatable(t(mut_cov_df), options = list(searching = FALSE, paging = FALSE))
```
...
read_cov_df <- data.frame(
  "Number of aligned reads" = coverage_df[4],
  "Total ref. genome coverage" = paste(round(as.integer(coverage_df[5]), 1), "%"),
  "Mean read depth" = round(as.integer(coverage_df[6])),
  check.names = FALSE
) # TODO more clear title stating which coverage

datatable(t(read_cov_df), options = list(searching = FALSE, paging = FALSE))
```
...
# not sure about the funct of the dataframe here, but it works so I leave it for now
write.csv2(coverage_df, file.path(
  dirname(params$coverage_file),
  "report_download_coverage.csv"
))
