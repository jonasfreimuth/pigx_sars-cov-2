  ...
  count_frame <- data.frame(
    ...
    total_sigmuts = as.numeric(
      rowSums(
        !is.na(
          mutations_ps %>%
            dplyr::select(dplyr::contains(mutation_sheet.v))
        )
      )
    ),
  ...
  for (var in colnames(mutation_sheet.df)) {
    count_frame[, paste0("sigmuts_", var)] <- as.numeric(
      rowSums(
        !is.na(
          mutations_ps %>%
            dplyr::select(dplyr::contains(na.omit(mutation_sheet.df[[var]])))
        )
      )
    )
  }
...
write_mutations_count <- function(mutation_plot_data,
                                  mutation_sheet.df,
                                  mutations_sig) {
  #' takes data_mut_plot.csv df, mutation_sheet.df with NAs at empty cells,
  ...
  sigmuts_found.df <- mutation_plot_data %>%
    dplyr::select(dplyr::contains(mutation_sheet.v))
  ...
  if (length(mutations_sig) > 0) {
    ...
    counts_per_sample <- do.call(
      bind_rows,
      apply(mutation_plot_data, 1, count_muts, mutation_sheet.df)
    )
